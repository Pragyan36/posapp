...
class DieselPage extends ConsumerStatefulWidget {
  const DieselPage({super.key});

  @override
  ConsumerState<DieselPage> createState() => _DieselPageState();
}

class _DieselPageState extends ConsumerState<DieselPage> {
  ...
  Widget build(BuildContext context) {
    final dieselPrice = ref.watch(fuelPricesProvider);
    final details = ref.watch(storeDetailsProvider);
    final double dieselPricePerLiter = double.parse(
        dieselPrice.dieselPrice.isEmpty ? "0" : dieselPrice.dieselPrice);

    if (dieselPricePerLiter == 0) {
      Toasts.showFormFailure('You have not set the price of Diesel');
    }
    ...
    void _onAmountChanged(String value) {
      ...
      final double liters = amount / dieselPricePerLiter;
    ...
    void _onLitersChanged(String value) {
      ...
      final double amount = liters * dieselPricePerLiter;
    ...
    void _printInvoice() async {
      try {
        ...
        // Items
        await SunmiPrinter.printText(
            'Diesel  ${_litersController.text.isEmpty ? "0" : _litersController.text}    ${dieselPricePerLiter.toStringAsFixed(2)}   ${_amountController.text.isEmpty ? "0" : double.parse(_amountController.text).toStringAsFixed(2)}',
